...
export const catalogRepository: CatalogRepository = {
  listProducts: () => supabaseCatalogRepository.listProducts(),
  getProductBySlug: (slug) => supabaseCatalogRepository.getProductBySlug(slug),
};
...
export async function createPendingOrder(
  ...
  const productsBySlug = new Map(
    (await catalogRepository.listProducts()).map((product) => [
      product.slug,
      product,
    ]),
  );
  ...
  const { data: order, error } = await client
    ...
    .insert({
      ...
      required_date: input.customer.requiredDate,
      delivery_address: input.customer.address,
      notes: input.customer.notes ?? null,
      status: "pending",
      total_minor: total,
    })
    .select("id, reference, total_minor")
    .single();
  if (error) throw error;
  const { error: itemError } = await client.from("order_items").insert(
    resolved.map(({ product, size, quantity }) => ({
      order_id: order.id,
      product_id: null,
      product_name: product.name,
      size_label: size.label,
      unit_price_minor: size.price,
      quantity,
    })),
  );
  ...
  return {
    ...
    items: resolved.map(({ product, size, quantity }) => ({
