...
export default function ProductPage({ params }: { params: { slug: string } }) {
  ...
  const size =
    product.sizes.find((option) => option.label === selectedSize) ??
    product.sizes[0];
  const addToCart = () => {
    const current = JSON.parse(localStorage.getItem("cakes-cart") ?? "[]");
    const existing = current.find(
      (item: { slug: string; size: string }) =>
        item.slug === product.slug && item.size === size.label,
    );
    ...
    <main className="detail-page">
      <div className="detail-shell">
        ...
        <div className="detail-grid">
          ...
          <div className="detail-copy">
            ...
            <p className="eyebrow">Made to order in Dahisar</p>
            <h1>{product.name}</h1>
            ...
            <div className="detail-total">
              <span>Total</span>
              <strong>{formatPrice(size.price * quantity)}</strong>
            </div>
            <button className="detail-button" onClick={addToCart}>
              Add to cart & continue
            </button>
            <p className="detail-note">
              Your order is confirmed personally on WhatsApp after we check
      ...
      <style jsx global>{`
        ...
        .detail-image img {
          ...
          object-fit: cover;
          border-radius: 24px;
          box-shadow: 0 16px 45px rgba(68, 35, 48, 0.11);
        }
        .detail-copy h1 {
          margin: 7px 0 16px;
          font:
            600 clamp(38px, 5vw, 64px)/1.05 Georgia,
            serif;
          color: #2e2428;
        }
        .detail-tag {
        ...
        .size-option.selected {
          border-color: #7b3154;
          background: #f6e7ed;
          box-shadow: 0 0 0 2px rgba(123, 49, 84, 0.12);
        }
        .size-option strong {
          color: #7b3154;
        }
