...
type Settings = {
  ...
  site_copy: Record<string, string>;
};
...
export default function AdminPage() {
  ...
  const [newProduct, setNewProduct] = useState({
    ...
    details: "",
    imageUrl: "",
    sizes: [{ label: "500 g", price: "" }],
  });
  const [photo, setPhoto] = useState<File | null>(null);
  const [isProductModalOpen, setIsProductModalOpen] = useState(false);
  const [activeSection, setActiveSection] = useState<
    "catalog" | "orders" | "feedback" | "settings"
  >("catalog");
  ...
  async function loadDashboard() {
    ...
    if (
      ...
      setError(
        "Your account is not registered as an admin. Add its Auth user ID to public.admin_users.",
      );
      return;
    }
    setProducts(await responses[0].json());
    setOrders(await responses[1].json());
    setFeedback(await responses[2].json());
    const settingRows = await responses[3].json();
    setSettings(
      settingRows.reduce(
        (
          result: Settings,
          row: { key: "business" | "site_copy"; value: Record<string, string> },
  ...
  const updateNewSize = (
    ...
    setNewProduct({
      ...newProduct,
      sizes: newProduct.sizes.map((size, sizeIndex) =>
  ...
  async function createProduct(event: React.FormEvent) {
    ...
    form.append("tag", newProduct.tag);
    form.append("description", newProduct.description);
    form.append("details", newProduct.details);
    ...
    if (!response.ok) {
      setError((await response.json()).error ?? "Unable to create product.");
      return;
    }
    setNewProduct({
      name: "",
    ...
    return (
      <main className="admin-page">
        <form className="admin-login" onSubmit={login}>
          <p className="eyebrow">Cakes n&apos; Shapes</p>
          <h1>Admin sign in</h1>
          <p>Manage the menu, orders and customer feedback.</p>
          <label>
            Email
            <input
              required
        ...
        <div className="admin-workspace">
          <nav className="admin-nav" aria-label="Dashboard sections">
            {[
              ...
              ["orders", "Orders", `${orders.length} orders`],
              ["feedback", "Feedback", `${feedback.length} submissions`],
              ["settings", "Settings", "Storefront details"],
            ].map(([id, label, detail]) => (
              <button
                key={id}
            ...
            {activeSection === "catalog" && (
              <section className="admin-section">
                <div className="admin-section-heading">
                  ...
                  <button
                    className="admin-button add-product-button"
                    type="button"
                    ...
                    >
                      <div className="modal-heading">
                        <div>
                          ...
                          <p>Create a new item for the storefront catalog.</p>
                        </div>
                        <button
                      ...
                      <form className="product-create" onSubmit={createProduct}>
                        <div className="create-grid">
                          ...
                          <label className="wide">
                            Image URL <span>(optional if uploading)</span>
                            <input
                        ...
                        <div className="new-sizes">
                          ...
                          {newProduct.sizes.map((size, index) => (
                            <div className="size-line" key={index}>
                              <input
                                required
                      ...
                      <div className="admin-main">
                        ...
                        <div className="size-editor">
                          {product.product_sizes.map((size) => (
                            <div className="size-line" key={size.id}>
                              <input
                                defaultValue={size.label}
                                aria-label={`${product.name} size`}
                                onBlur={(event) =>
                                  event.target.value !== size.label &&
                              ...
                              <input
                                type="number"
                                defaultValue={size.price_minor}
                                aria-label={`${product.name} price`}
                                onBlur={(event) =>
                                  update("/api/admin/products", {
                                    sizeId: size.id,
                                    price_minor: Number(event.target.value),
                                  })
                                }
                              />
                              <button
                    ...
                    orders.map((order) => (
                      <div className="admin-row" key={order.id}>
                        <div>
                          ...
                          <small>
                            {order.customer_phone} · {order.required_date}
                          </small>
            ...
            {activeSection === "feedback" && (
              <section className="admin-section">
                <div className="admin-section-heading">
                  ...
                  <span>{feedback.length} submissions</span>
                </div>
                <div className="admin-table">
                  {feedback.length ? (
                    feedback.map((item) => (
                    ...
                    <p className="empty-admin">No feedback submissions yet.</p>
                  )}
                </div>
              </section>
            )}
                ...
                <div className="settings-grid">
                  <label>
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setSetting("business", "name", event.target.value)
                      }
                    />
                  </label>
                  <label>
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setSetting("business", "location", event.target.value)
                      }
                    />
                  </label>
                  <label>
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setSetting("business", "phone", event.target.value)
                      }
                    />
                  </label>
                  <label>
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setSetting("business", "whatsapp", event.target.value)
                      }
                    />
                  </label>
                  <label>
                    Homepage title
                    <input
                      value={settings.site_copy.title ?? ""}
                      onChange={(event) =>
                        setSetting("site_copy", "title", event.target.value)
                      }
                    />
                  </label>
                  <label>
                    Baker name
                    <input
                      value={settings.site_copy.baker ?? ""}
                      onChange={(event) =>
                        setSetting("site_copy", "baker", event.target.value)
                      }
                    />
                  </label>
                  <button
                    className="admin-button settings-save"
                    onClick={() => {
  ...
  return (
    <style jsx global>{`
      ...
      .product-modal {
        ...
        border-radius: 16px;
        background: #fff;
        box-shadow: 0 24px 70px rgba(46, 36, 40, 0.2);
      }
      .modal-heading {
        display: flex;
        align-items: start;
